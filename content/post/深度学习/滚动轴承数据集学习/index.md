...
#### 单纯依靠‘振动幅值’只能做到整体的阈值报警，无法洞察退化的本质

​	引入‘频率’维度，是因为不同退化阶段，会在特定的频带上呈现出截然不同的能量分布。例如早期微弱故障通常激发高频共振，而晚期失效则表现为全频段的宽带噪声。

​	CWT 提供的二维时频图，将幅值转化为颜色能量，将时间与频率在 XY 轴展开。这使得我们的 CNN 模型不再是简单地‘看绝对响度’，而是真正学会了‘听音辨病’，通过捕捉特定频带上能量纹理的演变规律，实现了高鲁棒性、高精度的剩余寿命预测。
...
- **`data` 代表公式里的 $x(t)$**：它是你要分析的原始信号（源数据）
- **`第二个输入` 代表公式里的 $a$（尺度）**：它是你人为设定的一组数字，决定了你要把小波拉伸或压缩成多少种不同的胖瘦程度（对应不同的频率）。
